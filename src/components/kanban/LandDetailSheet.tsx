...
export function LandDetailSheet({ landId, onClose }: LandDetailSheetProps) {
  const { toast } = useToast()
  ...
  const fetchData = async () => {
    if (!landId) return
    try {
      const data = await pb.send(`/backend/v1/lands/${landId}`, { method: 'GET' })
      ...
      try {
        ...
        const query = clusterSerial
          ? `external_id="${landId}" || external_id="${clusterSerial}"`
          : `external_id="${landId}"`
        const meta = await pb
  ...
  useRealtime('land_metadata', (e) => {
    if (e.record.external_id === landId || (land && e.record.external_id === land.clusterSerial)) {
      fetchData()
    }
  })
  ...
  const fetchHistoryLogs = async () => {
    const clusterSerial = land?.clusterSerial || land?.external_id || land?.externalId || landId
    if (!clusterSerial && !landId) return

    try {
      const queryId = clusterSerial
        ? `land_id="${landId}" || land_id="${clusterSerial}"`
        : `land_id="${landId}"`
  ...
  useRealtime('history_logs', (e) => {
    const clusterSerial = land?.clusterSerial || land?.external_id || land?.externalId || landId
    if (e.record.land_id === landId || e.record.land_id === clusterSerial) {
      fetchHistoryLogs()
    }
  })
  ...
  useEffect(() => {
    ...
    const code =
      land?.clusterSerial || land?.external_id || land?.externalId || land?.code || landId
    if (!code) {
